...
import {getMentorshipsByMenteeId, getMentorshipsByMentorId} from "../../util/apiCall/MentorshipApiCall";
...
const MentorList = () => {
    ...
    useEffect(() => {
        if (principals) {
            getMentorshipsByMenteeId(principals.id)
                .then(response => {
        ...
        <div>
            <h5>Mentor List</h5>
            {
                ...
                mentorships.map(mentorship => (
                    <Panel key={mentorship.id} shaded className='p-3 mb-3 bg-light'>
                        <div className='mb-3'>{mentorship.mentor.account.name}</div>
            ...
            }
            {
                ...
                mentorships.length === 0 && (
                    <Message color='teal'>
                        Henüz mentee olarak katıldığınız bir program yok
                    </Message>
                )
            }
            {
